...
func TestSingleKingsOnEndGameEvaluation(t *testing.T) {
	...
	pos.AddPiece(board.BlackKing, "d4") // King on d4 +27
	pos.AddPiece(board.WhiteKing, "e1") // King on e1 -28
...
func TestWhitePawnNearQueening(t *testing.T) {
	pos := board.EmptyPosition()
	pos.AddPiece(board.WhitePawn, "d7") // Pawn on 7rank +134 (endgame) + 94 pawn value
...
func TestBlackPawnNearQueening(t *testing.T) {
	pos := board.EmptyPosition()
	pos.AddPiece(board.BlackPawn, "a2") // Pawn on 2rank +178 (a7 from white's view)
