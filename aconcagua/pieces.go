...
func raysDirection(square Bitboard, direction uint64) Bitboard {
	...
	return raysAttacks[direction][Bsf(square)] | square |
		raysAttacks[oppositeDirections[direction]][Bsf(square)]
}
...
func getRayPath(from *Bitboard, to *Bitboard) (rayPath Bitboard) {
	...
	return raysAttacks[fromDirection][fromSq] &
		raysAttacks[toDirection][toSq]
}
...
// the path or capture the checking piece when in check
// TODO: Optimization just to run once for all moves...
// func checkRestrictionsSquares(king Bitboard, checkingPieces Bitboard)
