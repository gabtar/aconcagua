  ...
  if pos.Check(b.color) {
    ...
    if len(checkingPieces) == 1 {
      ...
      if checker.IsSliding() {
        direction := getDirection(checker.Square(), pos.KingPosition(b.color))
        directionRay := raysDirection(pos.KingPosition(b.color), direction)

        moves |= directionRay & posiblesMoves
      }
    } else {
